---
title: "如何让 Claude Code 高效写测试？3 种工作流与避坑指南"
module: m2-thinking
order: 10
group: "质量保障"
description: "**Claude Code 让测试的边际成本降到近乎为零，测试真正变成了每步标配。** 3 种工作流：先实现后测试、TDD 先测试后实现、测试驱动重构。关键技巧：在 prompt 中列出 3-5 个测试场景，包含至少 2 个异常情况。"
duration: "18 分钟"
level: "需编程基础"
...
**因为 Claude Code 写测试的速度和写业务代码一样快，测试的边际成本降到近乎为零，不再需要在「赶进度」和「写测试」之间做取舍。** 说句实话，在没有 AI 工具之前，我对写测试这件事的态度是「知道应该写，但总是找借口不写」。理由总是很充分的：功能都还没写完呢，先赶进度；这段逻辑很简单，不会出问题的；测试写起来太烦了，改一下实现就得改一堆测试……
...
## Claude Code 写测试有哪三种工作流？

**三种工作流分别是：先实现后测试、TDD 先测试后实现、测试驱动重构，根据场景选择最合适的。** 根据场景不同，我在实际开发中交替使用三种工作流。没有哪种绝对更好，关键是选择适合当前情况的那个。
...
**AI 写的测试同样需要审查，重点关注三个问题：测试场景是否重复覆盖同一路径、断言是否耦合实现细节、Mock 是否导致「永远通过」。** 别忘了，AI 写的测试和它写的业务代码一样，需要你过一遍。我见过几种典型的测试质量问题：
