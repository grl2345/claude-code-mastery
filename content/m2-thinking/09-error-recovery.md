---
title: "Claude Code 犯错了怎么办？3 个错误等级与 3 种回退手段"
module: m2-thinking
order: 9
group: "渐进式开发"
description: "**AI 犯错是常态，关键技能是 30 秒内判断错误等级并选择对应策略。** 3 个等级：小瑕疵直接修、逻辑偏差明确指出、方向错误果断回退。3 种回退手段：git reset、新对话重试、换拆解方式。"
duration: "12 分钟"
level: "需编程基础"
...
**因为 AI 编程的真正技能不是「避免所有错误」，而是「快速从错误中恢复」——关键是 30 秒内判断错误等级并选择处理策略。** 我想先帮你调整一个心理预期：Claude Code 一定会犯错。不是「偶尔犯错」，而是「经常犯错」。如果你指望它每次都产出完美代码，你会很痛苦。
...
**分为 3 个等级：小瑕疵（30 秒直接修）、逻辑偏差（明确指出问题和预期）、方向错误（果断回退重来）。** 不是所有错误都值得同等对待。我把 AI 犯的错分成三个等级，每个等级对应不同的处理策略。
...
**Git 回退（最干净）、新对话重试（清除污染上下文）、换拆解方式（解决根本问题）。** 判断好错误等级之后，接下来就是执行回退。根据严重程度不同，我常用三种手段。
...
**把每次有代表性的错误记录到 CLAUDE.md 里，下次对话自动生效，同一个错误不会再出现第二次。** 每次 Claude Code 犯的错误都是有价值的信息。第一次遇到某个坑你可能要花十分钟才反应过来，但如果你把这个经验记下来，下次就能在 30 秒内识别。
