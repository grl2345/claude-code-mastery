---
title: "如何高效管理 Claude Code 的对话上下文？5 个实用策略"
module: m2-thinking
order: 5
group: "上下文工程"
description: "**用 5 个策略解决 80% 的上下文问题：单一职责对话、精确文件引用、摘要接力、分层加载、/compact 压缩。** 其中策略 1 和策略 2 优先级最高，先养成这两个习惯即可。"
duration: "15 分钟"
level: "需编程基础"
...
## 为什么「一次对话只做一件事」是最重要的策略？

**因为 Claude Code 在上下文干净的时候表现最好，混入不相关任务会让两个任务的质量都下降。** 这是最重要的一条，也是最容易被忽视的一条。
...
**把「看看 src/modules/ 目录」换成「看 src/modules/auth/service.ts 中的 validateToken 函数」，上下文消耗可减少十倍以上。**
...
**在当前对话结束前让 Claude 总结进展，把 200 字摘要复制到新对话开头，传达的有效信息比 8 轮完整对话还多。** 对话进行到七八轮以上的时候，你可能会发现 Claude 开始遗忘前面说好的事情，或者回复变得不那么精准。这时候不要硬撑，用摘要接力的方式切换到新对话。
...
## 怎样让 Claude 自己判断需要读哪些文件？

**先给高层信息（技术栈、模块、目标），让 Claude 自己判断需要加载的文件范围，比你手动指定更精准。** 这个策略的思路跟写代码很像：先给抽象层，再给具体实现。
...
**/compact 适合在对话变长、回复开始啰嗦重复时使用，它会自动压缩历史为精简摘要继续对话。** `/compact` 是 Claude Code 自带的上下文压缩命令。当你感觉对话已经进行了不少轮、或者 Claude 的回复开始变得啰嗦、重复的时候，就是使用它的好时机。
