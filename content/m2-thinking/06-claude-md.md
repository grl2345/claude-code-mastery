---
title: "CLAUDE.md 怎么写才有效？让每次对话自动获得项目上下文"
module: m2-thinking
order: 6
group: "上下文工程"
description: "**CLAUDE.md 是 Claude Code 的「入职手册」，写一次永久生效，省去每次重复交代项目背景。** 控制在 500 字以内，覆盖技术栈、代码规范、目录约定、常用命令 4 个关键信息，支持子目录分级配置。"
duration: "12 分钟"
level: "需编程基础"
publishedAt: 2026-03-25
---

## 为什么 CLAUDE.md 是上下文管理的秘密武器？

**因为它能让你写一次项目背景信息，之后每次对话自动生效，省掉大量重复交代。** 前面几课我们一直在聊怎么手动管理上下文，但有一个东西能让你省掉大量重复劳动：CLAUDE.md。
...
## CLAUDE.md 应该包含哪些关键信息？

**一份好的 CLAUDE.md 需要覆盖 4 个关键信息：项目概览、技术栈（带版本号）、代码规范、目录结构与常用命令。** 不需要长篇大论，控制在 500 字以内。
...
## 项目变大后一份 CLAUDE.md 不够用怎么办？

**在子目录放各自的 CLAUDE.md，Claude Code 会同时读取根目录和当前子目录的配置，实现「继承 + 覆盖」。** 随着项目变大，你可能会发现一份 CLAUDE.md 不太够用。比如支付模块有一些很具体的规范（所有金额使用整数表示，单位是分，不使用浮点数），放在根目录的 CLAUDE.md 里会显得臃肿，而且其他模块的对话也不需要这些信息。
