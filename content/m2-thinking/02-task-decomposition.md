---
title: "如何将模糊需求变成可执行指令？任务拆解的三层模型"
module: m2-thinking
order: 2
group: "任务拆解"
description: "**用「目标层 → 步骤层 → 验证层」三层模型把模糊需求变成可执行指令。** 目标层定义可验证的完成标准，步骤层拆出每次独立对话的任务，验证层确保每一步做对再继续。"
duration: "15 分钟"
level: "需编程基础"
...
**因为没有框架的拆解就像凭感觉写代码——要么遗漏关键步骤，要么拆得太粗导致 Claude 还是在猜你的意图。**

我自己摸索了一段时间后，总结出一个简单但好用的拆解框架：**目标层 → 步骤层 → 验证层。** 这三层不是什么高深理论，本质上就是回答三个问题：做完了长什么样？分几步走？每一步怎么确认做对了？

## 目标层应该怎么写？——做完之后，世界有什么不同？

**目标层要回答的核心问题是：做完这件事之后，你能用什么方式判断它「做到了」？** 关键是描述结果而非动作。
...
**目标定清楚了，接下来拆步骤，关键原则是每一步对应一次独立的 Claude Code 对话。** 继续用 auth 重构的例子：
...
## 验证层为什么不能省？——怎么确认每一步做对了？

**步骤拆好了还不够，每一步都需要独立验证，否则出了问题你根本不知道是哪一步引入的。** 这一层很多人会忽略，觉得「做完跑一下不就行了」。但如果等到最后才验证，排查成本会指数级增长。
