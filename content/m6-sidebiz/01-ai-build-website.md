---
title: "怎么用 Claude Code 搭建副业网站？从零到月入 4300 元完整记录"
module: "m6-sidebiz"
order: 1
group: "网站变现"
description: "用 Claude Code + Next.js + MDX 搭建内容网站，15 分钟生成项目骨架，6 个月达到日均 1200 UV、月入 4300 元（AdSense 2800 + 联盟 1500）。本文覆盖选方向、技术搭建、内容创作、SEO 优化、部署自动化全流程，附真实数据和 4 个踩坑教训。"
duration: "25 分钟"
level: "零基础可读"
publishedAt: "2026-04-05"
---

# 怎么用 Claude Code 搭建副业网站？从零到月入 4300 元完整记录

## 为什么要用 AI 搭建副业网站？

**用 Claude Code 搭建副业网站可以大幅降低技术门槛，让不懂前端、设计、SEO 的后端开发者也能在 6 个月内做到日均 1200 UV、月入 4300 元。** 2025 年中，我在一家中型公司做后端开发，薪资还行，但总觉得收入结构太单一。万一哪天被裁，除了投简历没有任何后路。
...
**选方向的核心逻辑：选自己有实际经验的领域 + 有搜索量但竞争不太激烈的细分 + 能对接广告或联盟营销。** 做内容网站，方向比技术重要得多。我花了大概一周时间调研，核心逻辑是：
...
## 怎么用 Claude Code 快速搭建网站框架？

**用 Next.js + MDX 技术栈，Claude Code 15 分钟生成完整项目骨架（目录结构、配置文件、布局组件、MDX 解析），手动搭建至少要两三天。** 我选的技术栈是 Next.js + MDX，原因是：
...
## 用 AI 做副业网站的内容创作流程是什么？

### 我的写作流程

**不是让 AI 直接写文章，而是自己积累真实体验→列大纲→AI 组织语言→自己删套话加观点→AI 检查准确性。有了模板后每篇文章从一天缩短到三四个小时。** 很多人以为用 AI 做内容就是让 AI 直接写文章。我的做法完全不同：
...
**Claude Code 可自动生成 sitemap.xml、Open Graph 标签、结构化数据标记等技术 SEO 配置；内容 SEO 的关键是每篇文章针对一个长尾关键词，上线两个月开始有自然流量，第四个月日均 UV 达 500。** Claude Code 在 SEO 技术优化上帮了大忙：
...
**Vercel 部署不到 10 分钟（推代码→自动构建→绑域名→自动 SSL），再用 Claude Code 写自动化脚本（发布、监控、备份）共约 500 行代码。** 部署流程非常简单：
...
**6 个月数据：42 篇文章、日均 1200 UV、月 AdSense 收入约 2800 元、月联盟营销收入约 1500 元，每周投入约 8 小时。** 网站上线六个月后的数据：
...
**四大坑：AI 直接写文章导致 Google 收录慢、技术选型过度复杂、忽视页面加载速度（5 秒→优化到 1.5 秒）、不重视移动端体验（60% 流量来自手机）。**
