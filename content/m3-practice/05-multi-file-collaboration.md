---
title: "如何用 Claude Code 开发跨 7 个文件的全栈功能？多文件协作实战"
module: m3-practice
order: 5
group: "多文件协作"
description: "**用 Claude Code 为 Express + React 项目添加通知功能，12 次对话完成 7 个新增文件和 3 个修改文件，效率提升约 50%。** 按依赖顺序从 Model 到 Service 到前端组件逐层构建，含联调踩坑（路由前缀重复、proxy 配置）和 5 条多文件协作经验。"
duration: "25 分钟"
level: "需编程基础"
publishedAt: 2026-04-05
---

## 什么样的需求适合用 Claude Code 做多文件协作？

**"文件多但单个文件改动不大"的全栈联动场景最适合，Claude Code 能将模板代码的编写时间压缩一半以上。** 这次的需求是给一个内部项目管理工具加一个"站内通知"功能。需求不复杂，但涉及的面比较广：
...
**先列出所有需要新增和修改的文件清单，然后按依赖顺序从底层往上逐步构建。** 开始之前，我先列了一份清单：
...
## Claude Code 生成数据模型和迁移文件靠谱吗？（对话 1-2）

**非常靠谱，关键是在 prompt 中指定"参考现有文件的写法"，确保风格一致。** 数据模型和迁移文件是固定模板类代码，Claude Code 两次对话都一次成功。
...
## Claude Code 写后端服务和路由时需要注意什么？（对话 3-5）

**注意审查性能写法——Claude Code 倾向于写"最直观"而非"最高效"的代码，如两次查询可合并为一次的场景。** 同时修改核心文件时要保持原子性，每次只做一处改动。
...
## Claude Code 生成前端组件常见的问题有哪些？（对话 6-9）

**最常见的问题是"和项目现有约定不一致"——时间格式、图标映射缺 default、setInterval 未清理等。** 这些问题都源于 Claude Code 不了解项目既有约定。
...
## 多文件联调最容易踩什么坑？（对话 10-12）

**联调问题几乎都是上下文缺失导致的——全局路由前缀、proxy 配置等"项目潜规则"不写在 CLAUDE.md 里就一定会踩坑。**
...
**12 次对话、3 小时完成，不用 Claude Code 估计需要 6-7 小时，效率提升约 50%。**
