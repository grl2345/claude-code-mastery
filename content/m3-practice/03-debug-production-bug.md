---
title: "如何用 Claude Code 排查 Node.js 线上内存泄漏？完整 Debug 实录"
module: m3-practice
order: 3
group: "排查与调试"
description: "**Node.js 服务内存 4 小时从 30% 涨到 78%，根因是批量导出功能的 Buffer.concat 全量内存分配。** 完整记录 3 轮排查过程：Claude Code 的初始分析找到了 taskMap 泄漏但非主因，第二轮给了错误方向浪费 30 分钟，最终靠 heapdump 对比定位真正根因并用流式写入修复。"
duration: "20 分钟"
level: "需编程基础"
...
**Node.js 服务内存使用率在 4 小时内从 30% 涨到 78%，且持续增长不回落，疑似与两天前上线的批量导出功能有关。** 某天下午，监控告警：一个 Node.js 服务的内存使用率在 4 小时内从 30% 涨到了 78%，而且还在缓慢增长。这个服务是一个内部的数据处理 API，跑在一台 2GB 内存的容器里，平时内存稳定在 500-600MB 左右。

看了一眼最近的部署记录，两天前上了一个版本，加了一个"批量导出"功能。直觉告诉我问题可能出在那里，但不确定。

## Claude Code 的静态代码分析能找到内存泄漏吗？
...
## Claude Code 排查内存泄漏时会帮倒忙吗？

**会。第二轮排查中它把方向引到 Express req 引用上，浪费了 30 分钟，最终证明方向完全错误。** 看起来问题找到了，但我想确认一下。于是我让 Claude Code 帮我加一段内存监控代码：
...
**用 heapdump 抓取 V8 heap snapshot 做对比分析，比让 AI 猜测可靠得多。** 我决定不再依赖 Claude Code 的猜测，而是用最基本的工具来定位问题。
...
**核心改动是将 Excel 生成从"全量内存 Buffer.concat"改为"流式逐行写入"，同时给 taskMap 加 10 分钟自动清理。** 定位到根因后，修复方案就清晰了：
...
**通过功能验证、内存验证和并发验证三轮测试确认：连续导出 20 次后内存稳定在 550MB，不再持续增长。** 修复部署后，我做了三轮验证：
...
## Claude Code 在 Debug 流程中应该扮演什么角色？

**Claude Code 最适合做"工具手"——你定排查方向，它写工具代码和执行修复；排查方向必须由开发者根据运行时证据来定。** 回看整个排查过程（总共约 2.5 小时），Claude Code 的表现可以分为三档：
