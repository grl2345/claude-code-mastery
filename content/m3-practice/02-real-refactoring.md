---
title: "如何用 Claude Code 重构 400 行的巨型组件？React God Component 拆分实录"
module: m3-practice
order: 2
group: "项目搭建"
description: "**用 Claude Code 将 427 行 React God Component 拆分为 9 个文件，耗时 3 小时，主文件降至 63 行。** 完整记录拆分策略、逐步提取过程，以及 useEffect 依赖导致的分页 Bug 排查经过。"
duration: "25 分钟"
level: "需编程基础"
publishedAt: 2026-04-02
---

## 什么样的代码需要用 Claude Code 重构？

**当一个文件超过 400 行、混合了 6 个功能区块和 11 个 useState 时，就是典型的重构目标。** 接手了一个内部的管理后台项目。React + TypeScript，前任开发者走得急，留下了一个 `DashboardPage.tsx`——**427 行，包含 6 个功能区块，混着数据获取、状态管理和 UI 渲染，全部塞在一个文件里。**

这种文件大家都见过。没人想碰它，但每次改需求都绕不过去。我决定趁一个相对空闲的下午，用 Claude Code 把它拆了。

完整过程花了 **3 个小时**，拆成了 **9 个文件**。中间有一次差点把线上功能搞挂，具体怎么回事后面会讲。

## Claude Code 的代码分析能力靠谱吗？

**结构梳理非常靠谱，但涉及运行时行为（竞态、并发、时序）的问题经常识别不到。** 开工之前我没有急着拆。先让 Claude Code 帮我梳理这个文件的结构：
...
**拆分策略必须由开发者自己定，因为它属于"影响大、难回退"的架构决策。** 基于分析结果，我规划了拆分方案。这个方案是我自己定的，没有让 Claude Code 来决定，原因在[架构决策那篇](/m2/11-architecture)里讲过——拆分策略属于"影响大、难回退"的决策，必须自己拿主意。
...
**从最简单、依赖最少的纯展示组件开始拆，逐步推进到有状态和副作用的组件。** 按照[小步快跑](/m2/07-small-steps)的原则，我从最简单、依赖最少的组件开始。
...
## Claude Code 提取 Hook 时容易引入什么 Bug？

**最常见的问题是改变副作用的触发方式——它倾向于用 useEffect "统一管理"副作用，可能引入引用不稳定导致的无限循环。** 这是整个重构里最关键也最危险的一步。
...
**将主文件变成纯组合层，目标控制在 80 行以内，确保 TypeScript 编译和现有测试全部通过。** 最后一步是把 DashboardPage.tsx 变成纯组合层：
